(this["webpackJsonpemployee-directory"] =
  this["webpackJsonpemployee-directory"] || []).push([
  [0],
  {
    13: function (n, a, e) {},
    14: function (n, a) {
      n.exports = JSON.parse(
        '[{\n  "id": 1,\n  "first_name": "Verine",\n  "last_name": "Beddo",\n  "email": "vbeddo0@columbia.edu",\n  "state": "Colorado"\n}, {\n  "id": 2,\n  "first_name": "Ofella",\n  "last_name": "McLeman",\n  "email": "omcleman1@wiley.com",\n  "state": "Colorado"\n}, {\n  "id": 3,\n  "first_name": "Sissie",\n  "last_name": "Elphinston",\n  "email": "selphinston2@discovery.com",\n  "state": "Colorado"\n}, {\n  "id": 4,\n  "first_name": "Nevsa",\n  "last_name": "Guidoni",\n  "email": "nguidoni3@spotify.com",\n  "state": "Colorado"\n}, {\n  "id": 5,\n  "first_name": "Clareta",\n  "last_name": "Baggally",\n  "email": "cbaggally4@exblog.jp",\n  "state": "Colorado"\n}, {\n  "id": 6,\n  "first_name": "Stevy",\n  "last_name": "Ledner",\n  "email": "sledner5@fastcompany.com",\n  "state": "Washington"\n}, {\n  "id": 7,\n  "first_name": "Benyamin",\n  "last_name": "McMyler",\n  "email": "bmcmyler6@youtu.be",\n  "state": "Colorado"\n}, {\n  "id": 8,\n  "first_name": "Bonnibelle",\n  "last_name": "Anan",\n  "email": "banan7@apple.com",\n  "state": "Washington"\n}, {\n  "id": 9,\n  "first_name": "Neda",\n  "last_name": "Bartosch",\n  "email": "nbartosch8@paypal.com",\n  "state": "Colorado"\n}, {\n  "id": 10,\n  "first_name": "Joseito",\n  "last_name": "Lansbury",\n  "email": "jlansbury9@stanford.edu",\n  "state": "Colorado"\n}, {\n  "id": 11,\n  "first_name": "Sheena",\n  "last_name": "MacLleese",\n  "email": "smaclleesea@statcounter.com",\n  "state": "Colorado"\n}, {\n  "id": 12,\n  "first_name": "Michaela",\n  "last_name": "Emerson",\n  "email": "memersonb@yolasite.com",\n  "state": "Colorado"\n}, {\n  "id": 13,\n  "first_name": "Lucais",\n  "last_name": "Emerton",\n  "email": "lemertonc@wired.com",\n  "state": "Washington"\n}, {\n  "id": 14,\n  "first_name": "Randie",\n  "last_name": "Anelay",\n  "email": "ranelayd@naver.com",\n  "state": "Colorado"\n}, {\n  "id": 15,\n  "first_name": "Christalle",\n  "last_name": "Makin",\n  "email": "cmakine@adobe.com",\n  "state": "Washington"\n}, {\n  "id": 16,\n  "first_name": "Ainslee",\n  "last_name": "Nutbeam",\n  "email": "anutbeamf@naver.com",\n  "state": "Arizona"\n}, {\n  "id": 17,\n  "first_name": "Carleton",\n  "last_name": "Point",\n  "email": "cpointg@bizjournals.com",\n  "state": "Washington"\n}, {\n  "id": 18,\n  "first_name": "Rockwell",\n  "last_name": "Portigall",\n  "email": "rportigallh@wikispaces.com",\n  "state": "Colorado"\n}, {\n  "id": 19,\n  "first_name": "Rupert",\n  "last_name": "Litchfield",\n  "email": "rlitchfieldi@google.com.br",\n  "state": "Colorado"\n}, {\n  "id": 20,\n  "first_name": "Brucie",\n  "last_name": "Cockshott",\n  "email": "bcockshottj@reddit.com",\n  "state": "Colorado"\n}, {\n  "id": 21,\n  "first_name": "Abba",\n  "last_name": "Bartholin",\n  "email": "abartholink@time.com",\n  "state": "Washington"\n}, {\n  "id": 22,\n  "first_name": "Alejandra",\n  "last_name": "Bache",\n  "email": "abachel@apple.com",\n  "state": "Washington"\n}, {\n  "id": 23,\n  "first_name": "Lia",\n  "last_name": "Snare",\n  "email": "lsnarem@mozilla.org",\n  "state": "Oregon"\n}, {\n  "id": 24,\n  "first_name": "Maddi",\n  "last_name": "Lardeux",\n  "email": "mlardeuxn@nydailynews.com",\n  "state": "Colorado"\n}, {\n  "id": 25,\n  "first_name": "Barbie",\n  "last_name": "Crombleholme",\n  "email": "bcrombleholmeo@alexa.com",\n  "state": "Oregon"\n}, {\n  "id": 26,\n  "first_name": "Ardelle",\n  "last_name": "Vassall",\n  "email": "avassallp@51.la",\n  "state": "Colorado"\n}, {\n  "id": 27,\n  "first_name": "Annissa",\n  "last_name": "Lochet",\n  "email": "alochetq@yellowbook.com",\n  "state": "Oregon"\n}, {\n  "id": 28,\n  "first_name": "Cary",\n  "last_name": "Petyt",\n  "email": "cpetytr@dropbox.com",\n  "state": "Colorado"\n}, {\n  "id": 29,\n  "first_name": "Marsh",\n  "last_name": "Maggiori",\n  "email": "mmaggioris@pinterest.com",\n  "state": "Colorado"\n}, {\n  "id": 30,\n  "first_name": "Dorree",\n  "last_name": "Rutherforth",\n  "email": "drutherfortht@google.es",\n  "state": "Colorado"\n}, {\n  "id": 31,\n  "first_name": "Carlin",\n  "last_name": "Djurkovic",\n  "email": "cdjurkovicu@qq.com",\n  "state": "Colorado"\n}, {\n  "id": 32,\n  "first_name": "Neely",\n  "last_name": "Shattock",\n  "email": "nshattockv@gmpg.org",\n  "state": "Colorado"\n}, {\n  "id": 33,\n  "first_name": "Terrence",\n  "last_name": "Fay",\n  "email": "tfayw@yelp.com",\n  "state": "Colorado"\n}, {\n  "id": 34,\n  "first_name": "Lay",\n  "last_name": "Tubbles",\n  "email": "ltubblesx@thetimes.co.uk",\n  "state": "Arizona"\n}, {\n  "id": 35,\n  "first_name": "Petronella",\n  "last_name": "Wimms",\n  "email": "pwimmsy@mac.com",\n  "state": "Colorado"\n}, {\n  "id": 36,\n  "first_name": "Alexis",\n  "last_name": "Patten",\n  "email": "apattenz@github.com",\n  "state": "Colorado"\n}, {\n  "id": 37,\n  "first_name": "Bari",\n  "last_name": "Radband",\n  "email": "bradband10@cargocollective.com",\n  "state": "Colorado"\n}, {\n  "id": 38,\n  "first_name": "Harmonia",\n  "last_name": "Deville",\n  "email": "hdeville11@forbes.com",\n  "state": "Colorado"\n}, {\n  "id": 39,\n  "first_name": "Lin",\n  "last_name": "Kerford",\n  "email": "lkerford12@prnewswire.com",\n  "state": "Colorado"\n}, {\n  "id": 40,\n  "first_name": "Jill",\n  "last_name": "Harrell",\n  "email": "jharrell13@pagesperso-orange.fr",\n  "state": "Colorado"\n}, {\n  "id": 41,\n  "first_name": "Kasper",\n  "last_name": "Ankers",\n  "email": "kankers14@lycos.com",\n  "state": "Colorado"\n}, {\n  "id": 42,\n  "first_name": "Farrell",\n  "last_name": "Warlawe",\n  "email": "fwarlawe15@java.com",\n  "state": "Colorado"\n}, {\n  "id": 43,\n  "first_name": "Harlan",\n  "last_name": "Brotherhood",\n  "email": "hbrotherhood16@ning.com",\n  "state": "Arizona"\n}, {\n  "id": 44,\n  "first_name": "Gabby",\n  "last_name": "Rawne",\n  "email": "grawne17@auda.org.au",\n  "state": "Arizona"\n}, {\n  "id": 45,\n  "first_name": "Roanna",\n  "last_name": "Iannuzzi",\n  "email": "riannuzzi18@bandcamp.com",\n  "state": "Colorado"\n}, {\n  "id": 46,\n  "first_name": "Fionnula",\n  "last_name": "Dingate",\n  "email": "fdingate19@youtu.be",\n  "state": "Colorado"\n}, {\n  "id": 47,\n  "first_name": "Andriette",\n  "last_name": "Kellar",\n  "email": "akellar1a@biblegateway.com",\n  "state": "Colorado"\n}, {\n  "id": 48,\n  "first_name": "Ella",\n  "last_name": "Lewinton",\n  "email": "elewinton1b@cbsnews.com",\n  "state": "Colorado"\n}, {\n  "id": 49,\n  "first_name": "Schuyler",\n  "last_name": "Le Brum",\n  "email": "slebrum1c@linkedin.com",\n  "state": "Arizona"\n}, {\n  "id": 50,\n  "first_name": "Elfreda",\n  "last_name": "Hancox",\n  "email": "ehancox1d@tinypic.com",\n  "state": "Arizona"\n}]'
      );
    },
    15: function (n, a, e) {
      "use strict";
      e.r(a);
      var t = e(0),
        i = e.n(t),
        l = e(2),
        r = e.n(l),
        s = (e(13), e(3)),
        m = e(4),
        o = e(6),
        c = e(5),
        d = e(7),
        f = e(14);
      console.log(f);
      var u = (function (n) {
        function a() {
          var n, e;
          Object(s.a)(this, a);
          for (var t = arguments.length, i = new Array(t), l = 0; l < t; l++)
            i[l] = arguments[l];
          return (
            ((e = Object(o.a)(
              this,
              (n = Object(c.a)(a)).call.apply(n, [this].concat(i))
            )).state = { dir: f }),
            (e.handleFirstAZ = function (n) {
              var a = JSON.parse(JSON.stringify(e.state.dir));
              a.sort(function (n, a) {
                return n.first_name.localeCompare(a.first_name);
              }),
                e.setState({ dir: a });
            }),
            (e.handleFirstZA = function (n) {
              var a = JSON.parse(JSON.stringify(e.state.dir));
              a.sort(function (n, a) {
                return a.first_name.localeCompare(n.first_name);
              }),
                e.setState({ dir: a });
            }),
            (e.handleLastAZ = function (n) {
              var a = JSON.parse(JSON.stringify(e.state.dir));
              a.sort(function (n, a) {
                return n.last_name.localeCompare(a.last_name);
              }),
                e.setState({ dir: a });
            }),
            (e.handleLastZA = function (n) {
              var a = JSON.parse(JSON.stringify(e.state.dir));
              a.sort(function (n, a) {
                return a.last_name.localeCompare(n.last_name);
              }),
                e.setState({ dir: a });
            }),
            (e.handleStateFilter = function (n) {
              var a;
              switch (n.target.value) {
                case "CA":
                  a = f.filter(function (n) {
                    return "Colorado" === n.state;
                  });
                  break;
                case "AZ":
                  a = f.filter(function (n) {
                    return "Arizona" === n.state;
                  });
                  break;
                case "NV":
                  a = f.filter(function (n) {
                    return "Nevada" === n.state;
                  });
                  break;
                case "WA":
                  a = f.filter(function (n) {
                    return "Washington" === n.state;
                  });
                  break;
                case "OR":
                  a = f.filter(function (n) {
                    return "Oregon" === n.state;
                  });
                  break;
                default:
                  a = f;
              }
              e.setState({ dir: a });
            }),
            e
          );
        }
        return (
          Object(d.a)(a, n),
          Object(m.a)(a, [
            {
              key: "render",
              value: function () {
                return i.a.createElement(
                  i.a.Fragment,
                  null,
                  i.a.createElement(
                    "div",
                    { className: "uk-padding-large uk-background-primary" },
                    i.a.createElement(
                      "h1",
                      { className: "uk-heading-medium uk-text-center" },
                      "Employee Directory"
                    )
                  ),
                  i.a.createElement(
                    "div",
                    { className: "uk-container" },
                    i.a.createElement(
                      "p",
                      { className: "uk-padding" },
                      "Sort by first name:",
                      i.a.createElement(
                        "button",
                        {
                          className:
                            "uk-button uk-button-default uk-button-small",
                          onClick: this.handleFirstAZ,
                        },
                        "A > Z"
                      ),
                      i.a.createElement(
                        "button",
                        {
                          className:
                            "uk-button uk-button-default uk-button-small",
                          onClick: this.handleFirstZA,
                        },
                        "Z > A"
                      ),
                      "Sort by last name:",
                      i.a.createElement(
                        "button",
                        {
                          className:
                            "uk-button uk-button-default uk-button-small",
                          onClick: this.handleLastAZ,
                        },
                        "A > Z"
                      ),
                      i.a.createElement(
                        "button",
                        {
                          className:
                            "uk-button uk-button-default uk-button-small",
                          onClick: this.handleLastZA,
                        },
                        "Z > A"
                      ),
                      "Filter by state:",
                      i.a.createElement(
                        "select",
                        {
                          className: "uk-select uk-form-width-small",
                          defaultValue: this.state.selectValue,
                          onChange: this.handleStateFilter,
                        },
                        i.a.createElement("option", { value: "All" }, "All"),
                        i.a.createElement("option", { value: "AZ" }, "Arizona"),
                        i.a.createElement(
                          "option",
                          { value: "CA" },
                          "Colorado"
                        ),
                        i.a.createElement("option", { value: "NV" }, "Nevada"),
                        i.a.createElement("option", { value: "OR" }, "Oregon"),
                        i.a.createElement(
                          "option",
                          { value: "WA" },
                          "Washington"
                        )
                      )
                    ),
                    i.a.createElement(
                      "table",
                      { className: "uk-table uk-table-striped" },
                      i.a.createElement(
                        "thead",
                        null,
                        i.a.createElement(
                          "tr",
                          null,
                          i.a.createElement("th", null, "First Name"),
                          i.a.createElement("th", null, "Last Name"),
                          i.a.createElement("th", null, "Email Address"),
                          i.a.createElement("th", null, "State")
                        )
                      ),
                      i.a.createElement(
                        "tbody",
                        null,
                        this.state.dir.map(function (n) {
                          return i.a.createElement(
                            "tr",
                            { key: n.id },
                            i.a.createElement("td", null, n.first_name),
                            i.a.createElement("td", null, n.last_name),
                            i.a.createElement("td", null, n.email),
                            i.a.createElement("td", null, n.state)
                          );
                        })
                      )
                    )
                  )
                );
              },
            },
          ]),
          a
        );
      })(t.Component);
      Boolean(
        "localhost" === window.location.hostname ||
          "[::1]" === window.location.hostname ||
          window.location.hostname.match(
            /^127(?:\.(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$/
          )
      );
      r.a.render(i.a.createElement(u, null), document.getElementById("root")),
        "serviceWorker" in navigator &&
          navigator.serviceWorker.ready
            .then(function (n) {
              n.unregister();
            })
            .catch(function (n) {
              console.error(n.message);
            });
    },
    8: function (n, a, e) {
      n.exports = e(15);
    },
  },
  [[8, 1, 2]],
]);
//# sourceMappingURL=main.cc63706f.chunk.js.map
